...
def collect_artifacts():
    artifacts = [os.path.join("files", f) for f in os.listdir('files')]
    artifacts.append(str('dist/'+package_name()+'-repo.json'))
...
def write_universe_files(release_version, artifacts_url, universe_path, force):
    config_values = _read_config_values(artifacts_url)
    ...
    if force or not os.path.exists(path+'/'+release_version):
